...
import type { HomeFeaturedOfferCard } from "@/components/home/HomeFeaturedOffersSection"
...
function buildLoopSlides(offers: HomeFeaturedOfferCard[]): HomeFeaturedOfferCard[] {
  if (offers.length === 0) return []
  const target = Math.max(18, offers.length * 4)
  const out: HomeFeaturedOfferCard[] = []
...
function OfferItem({ offer }: { offer: HomeFeaturedOfferCard }) {
  return (
    <motion.article
...
export function HomeFeaturedOffersCarousel({ offers }: { offers: HomeFeaturedOfferCard[] }) {
